...
async function api<T>(url: string, init?: RequestInit): Promise<T> { const response = await fetch(url, { ...init, cache: "no-store" }); const body = await response.json(); if (!response.ok) throw new Error(body.error ?? "Request failed."); return body; }
...
export default function Home() {
  ...
  async function chooseServer(id: string) { setServerId(id); setLibraryId(""); setMovies([]); if (!id) return; setLoading(true); try { const r = await api<{ libraries: PlexLibrary[] }>(`/api/plex/libraries?server=${encodeURIComponent(id)}`); setLibraries(r.libraries); setNotice("Choose a movie library."); } catch (e) { setNotice((e as Error).message); } finally { setLoading(false); } }
