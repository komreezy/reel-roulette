...
import { chooseConnection, classifyAddress, isApprovedConnectionUri, parsePayload, readLimitedBytes, validateArtworkPath } from "./plex";
...
describe("Plex payload and policy helpers", () => {
  ...
  it("classifies resolved destinations without exposing addresses", () => { expect(classifyAddress("192.168.1.5")).toBe("ipv4-nonpublic"); expect(classifyAddress("8.8.8.8")).toBe("ipv4-public"); expect(classifyAddress("fd00::1")).toBe("ipv6-nonpublic"); });
