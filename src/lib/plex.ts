...
export function isApprovedConnectionUri(uri: string) { try { const url = new URL(uri); const host = url.hostname.toLowerCase(); return url.protocol === "https:" && !url.username && !url.password && (host.endsWith(".plex.direct") || host.endsWith(".plex.services")) && (!url.port || url.port === "443" || url.port === "32400") && (url.pathname === "/" || url.pathname === "") && !url.search && !url.hash; } catch { return false; } }
export function chooseConnection(server: Pick<Resource, "rawConnections">) { const valid = server.rawConnections.filter(c => isApprovedConnectionUri(c.uri) && c.local !== true && c.local !== "1"); return valid.sort((a, b) => Number(a.relay === true || a.relay === "1") - Number(b.relay === true || b.relay === "1"))[0] ?? null; }
export async function resolveServer(token: string, clientId: string, id: string) { const server = (await getResources(token, clientId)).find(s => s.id === id); const connection = server && chooseConnection(server); if (!server || !connection) throw new PlexError("unreachable", "This server has no remotely reachable HTTPS connection."); return { uri: connection.uri.replace(/\/$/, ""), token: server.accessToken, machineIdentifier: server.machineIdentifier };
}
...
export async function getLibraries(connection: { uri: string; token: string }, clientId: string) { let response = await fetchWithTimeout(`${connection.uri}/library/sections`, connection.token, clientId); if (response.status === 404) response = await fetchWithTimeout(`${connection.uri}/library/sections/all`, connection.token, clientId); if (!response.ok) throw new PlexError("upstream", "Plex libraries could not be loaded."); return parsePayload(await read(response), "Directory").filter(i => i.type === "movie").map(i => ({ key: String(i.key), title: str(i.title) ?? "Movies", type: "movie" as const })); }
