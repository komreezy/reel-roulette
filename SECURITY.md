...
There is no database. Server connections are re-discovered from Plex resources for every route, require each resource’s own access token, and are restricted to non-local HTTPS connections on Plex-managed `plex.direct`, `plex.services`, or `relay.plex.tv` hosts and expected ports. Direct connections are reachability-probed before Relay fallback. Artwork accepts only validated relative metadata paths and is fetched server-side with `redirect:error`, timeouts, MIME limits, and streaming byte caps. Hosted Vercel deployments cannot reach LAN-only servers.
